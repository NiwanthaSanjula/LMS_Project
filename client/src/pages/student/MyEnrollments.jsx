...
const MyEnrollments = () => {
  ...
  const {  navigate, enrolledCourses, fetchUserEnrolledCourses, calculateCourseDuration,calculateNoOfLectures, userData, backendUrl, getToken, } = useContext(AppContext)
      ...
      <div className='md:px-36 px-8 pt-10'>
        ...
        {enrolledCourses.length === 0 ? (
          ...
          <table className='md:table-auto table-fixed w-full overflow-hidden mt-10'>
            <thead className='text-gray-900 text-left max-sm:hidden'>
              <tr>
              ...
              {enrolledCourses.map((course, index) => (
                <tr key={index} className='border-b border-gray-500/20'>
                  ...
                  <td className='px-4 py-3 max-sm:text-right'>
                    <button 
                      className={`w-20 sm:w-28 px-3 sm:px-5 py-1.5 sm:py-2 text-white max-sm:text-xs rounded-full cursor-pointer transition-all duration-200
                        ${
                          progressArray[index] && 
